 ...
 * Uses Web Audio API to process audio and logs raw data to console.
 */
export function useAudioCapture(): AudioCaptureReturn {
   ...
   * and logs it to the console using requestAnimationFrame.
   */
  const readAudioData = useCallback(() => {
    ...
    const tick = () => {
      ...
      // Calculate a simple RMS (Root Mean Square) for audio level
      let sum = 0;
